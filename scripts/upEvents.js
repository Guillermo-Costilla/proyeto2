...
let anio2022 = upevents(data.events, data.currentDate)
...
function seleccion() {
    ...
    filtrarCategory(anio2022, seleccionado)
}

let cardFiltrado = []
function filtrarCategory(anio2022, arrayInputs) {
    cardFiltrado = []
    anio2022.forEach(evento => {
        arrayInputs.forEach(
            elemento => {
    ...
    if (cardFiltrado.length > 0) {
        ...
        crearCards(anio2022, "#card-template")
    }
}
